...
Before you install {product-title}, you must provision two load balancers. The requirements for these load balancers are as follows:
...
1. *API Load Balancer*: Provides a common endpoint for users (both human and machine) to interact with and configure the platform.
  * Layer 4 load balancing only (this is sometimes referred to as Raw TCP, SSL Passthrough, or SSL Bridge mode)
  * Round robin load balancing
  * The following ports should be configured on both the front and back of the load balancers.
+
.API Load Balancer
...
2. *Application Ingress Load Balancer*: Provides an ingress point for application traffic flowing in from outside of the cluster.
  * Layer 4 load balancing only (this is sometimes referred to as Raw TCP, SSL Passthrough, or SSL Bridge mode)
  * Round robin load balancing
  * The following ports should be configured on both the front and back of the load balancers.
+
...
[TIP]
====
In some cases, enabling source IP-based session persistence may improve performance for applications using end-to-end TLS encryption. This assumes that the true IP of the client can be seen by the load balancer, which varies based on network setup.
====
